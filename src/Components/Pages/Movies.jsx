...
export default function Movies() {
  ...
  async function fetchMovies(pageNumber = 1) {
    try {
      const { data } = await axios.get(
        `${Defualt_Url}movie/popular?api_key=${Api_Key}&language=fa&page=${pageNumber}`
      );
  ...
  return (
    <div className="p-6 min-h-screen rtl">
      <h2 className="mb-8 font-bold text-yellow-600 text-3xl text-center">🎬 فیلم‌های محبوب</h2>
          ...
          <NavLink key={movie.id} to={`/movie/${movie.id}`}>
            <div className="bg-white shadow-lg hover:shadow-2xl rounded-xl transition hover:-translate-y-1 duration-300 transform">
              ...
              <div className="p-3 text-center">
                <h4 className="font-semibold text-gray-800 text-sm">{movie.title}</h4>
              </div>
            </div>
          </NavLink>
        ))}
